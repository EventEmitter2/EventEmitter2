...
class EventEmitter {
  ...
  constructor(conf = {}) {
    ...
    this._conf = conf
    if (conf.delimiter) this.delimiter = conf.delimiter;
    ...
    this.ignoreErrors      = conf.ignoreErrors      || false
    ...
    hasListeners(type) {
      if (this.wildcard) {
        return hasListeners___wildcard.call(this, type)
      } else {
        return hasListeners___classic.call(this, type)
      }
    }
  
    removeAllListeners(type) {
      if (this.wildcard) {
        removeAllListeners___wildcard.call(this, type)
      } else {
        removeAllListeners___classic.call(this, type)
      }
      return this;
    }
}
...
function emit() {
  ...
  if (handler.length + handlerAll.length) switch (arguments.length) {
    case 1 : 
      while (fn = handlerAll.shift()) { this.event = type; fn.call(this, type) }
      while (fn = handler   .shift()) { this.event = type; fn.call(this) }
      return true;
    case 2 : 
      while (fn = handlerAll.shift()) { this.event = type; fn.call(this, type, arguments[1]) }
      while (fn = handler   .shift()) { this.event = type; fn.call(this, arguments[1]) }
      return true;
    case 3 : 
      while (fn = handlerAll.shift()) { this.event = type; fn.call(this, type, arguments[1], arguments[2]) }
      while (fn = handler   .shift()) { this.event = type; fn.call(this,       arguments[1], arguments[2]) }
      return true;
    default: 
      const args_list = [ ...arguments ].slice(1)
      while (fn = handlerAll.shift()) { this.event = type; fn.apply(this, arguments) }
      while (fn = handler   .shift()) { this.event = type; fn.apply(this, args_list) }
      return true;
  }
...
function removeAllListeners___wildcard(type) {
    if (type === undefined) {
      this.listenerTree = this._events = {};
    } else {
      var leafs = searchListenerTree.call(this, null, type, this._events, 0), leaf, i;
      if (leafs) {
        for (i = 0; i < leafs.length; i++) {
          leaf = leafs[i];
          leaf._listeners = null;
        }
        recursivelyGarbageCollect(this._events);
      }
    }
}


function removeAllListeners___classic(type) {
  if (type === undefined) {
    this._events = {};
...
function hasListeners___wildcard(type) {
  var handlers = [];
  var ns = typeof type === 'string' ? type.split(this.delimiter) : type.slice();
  searchListenerTree.call(this, handlers, ns, this.listenerTree, 0);
...
function hasListeners___classic(type) {
  var _events = this._events;
  var _all = this._all;
