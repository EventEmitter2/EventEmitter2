;!function(root) {
  ...
  EventEmitter2.prototype.initializeEvents = function(conf) {
    ...
    if (conf.delimiter) {
      ...
      conf.eventDelimiter = conf.delimiter;
    }
    conf.eventDelimiter = conf.eventDelimiter || '.';
    self.setEventDelimiter(conf.eventDelimiter);
    self._eventCaseSensitive = typeof conf.eventCaseSensitive === 'undefined' ? true : conf.eventCaseSensitive;
  ...
  EventEmitter2.prototype.addListener = function(event, fn) {
    ...
    if (namespaced) {
      ...
      if (ns.some(function (name) {
          // for some (any) of the names have the ...
          // if there is a wildcard        but it's not a single character
          if (name.indexOf('*') !== -1 && name.length !== 1) {
            //end the some, return true
            return true;
          }})
         ) {
        self.eventNameError('addListener');
