...
;!function(undefined) {
  ...
  function listeners(type) {
    ...
    if (type === undefined) {
      ...
      const keys = ownKeys(_events);
      let i = keys.length;
      const allListeners = [];
      while (i-- > 0) {
        const item = _events[keys[i]];
        if (typeof item === 'function') {
          allListeners.push(item);
        } else {
          allListeners.push.apply(allListeners, item);
        }
      }
      return allListeners;
    } else {
      if (!_events || !_events[type]) return []
