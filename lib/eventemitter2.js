...
function emit() {
  ...
  if (this.wildcard) {
    ...
    if (type !== 'newListener' && type !== 'removeListener') {
      if (typeof type === 'object') {
        l = type.length;
        if (symbolsSupported) for (i = 0; i < l; i++) {
  ...
  if (this._all && this._all.length) {
    ...
    for (i = 0, l = handler.length; i < l; i++) {
      this.event = type;
      switch (al) {
        case 1:   handler[i].call(this, type);  break;
        case 2:   handler[i].call(this, type, arguments[1]);  break;
        case 3:   handler[i].call(this, type, arguments[1], arguments[2]);  break;
...
function _onAny(emitter, fn, prepend) {
  ...
  if (!emitter._all) emitter._all = [];

  // Add the function to the event listener collection.
  if (prepend) {
    emitter._all.unshift(fn);
  } else {
    emitter._all.push(fn);
  }
...
function _on(emitter, type, listener, prepend, options) {
  ...
  if (!emitter._events) init.call(emitter);
...
function eventNames(nsAsArray) {
  if (this.wildcard) {
    return collectTreeEvents.call(this, this.listenerTree, [], null, nsAsArray)
  } else {
    if (!this._events) return []
