...
function emit(...args) {
  ...
  if (this.wildcard) {
    ...
    if (type !== 'newListener' && type !== 'removeListener') {
      if (typeof type === 'object') {
        if (symbolsSupported) for (let i = 0, l = type.length; i < l; i++) {
    ...
    if (typeof d === 'function') handler.push(d)
    if (Array.isArray(d)) handler.push(...d)
  }


  for (let i = 0, l = handlerAll.length; i < l; i++) {
      this.event = type;
      switch (args.length) {
        case 0 :  handlerAll[i].call(this, type); break;
        case 1 :  handlerAll[i].call(this, type, args[0]);  break;
        case 2 :  handlerAll[i].call(this, type, args[0], args[1]);  break;
        default:  handlerAll[i].call(this, type, ...args);
      }
  }
  ...
  for (let i = 0, l = handler.length; i < l; i++) {
      this.event = type;
      switch (args.length) {
        case 0 : handler[i].call(this); break;
        case 1 : handler[i].call(this, args[0]); break;
        case 2 : handler[i].call(this, args[0], args[1]); break;
        default: handler[i].call(this, ...args);
      }
  }

  if (handler.length + handlerAll.length) return true;
  
  if (!this.ignoreErrors && type === 'error') {
