...
;!function(root) {
  ...
  EventEmitter2.prototype.addListener = function(event, listener) {
    ...
    // Set variables.
    var delimiter = this._eventDelimiter,
        caseSensative = this._eventCaseSensitive,
    ...
    if (event[event.length-1] === delimiter || event[0] === delimiter) {
      this.eventNameError('addListener');
      return false;
    }
  ...
  EventEmitter2.prototype.emit = function(event) {
    var self = this,
        args = arguments,
        i = 0,
        delimiter = this._eventDelimiter,
        caseSensitive = this._eventCaseSensitive,
        listeners = this._listeners,
