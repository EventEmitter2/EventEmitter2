...
var searchListenerTree = function(handlers, type, tree, i) {
  ...
  if(i === type.length && tree && tree[l]) {

    if(typeof tree[l] === 'function') {
      handlers.push(tree[l]);
    }
    else {
      for(var leaf = 0, len = tree[l].length; leaf < len; leaf++) {
        handlers.push(tree[l][leaf]);
      }
    }
  }
...
var growListenerTree = function(type, listener) {
  ...
  while (name) {
    ...
    if (type.length === 0) {
      ...
      else if('function' === typeof tree[l]) {
        tree[l] = [tree[l], listener];
      }
      else if (isArray(tree[l])) {

        tree[l].push(listener);

        if (!tree[l].warned) {

          var m = this.defaultMaxListeners;

          if (m > 0 && tree[l].length > m) {
